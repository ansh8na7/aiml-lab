...
#instantiate graph object with graph topology, heuristic values,start node
    ...
    def getStatus(self,v): # return the status of a given node
        return self.status.get(v,0) # 0 means needs exploration
    ...
    def printSolution(self):
        ...
        print("FOR GRAPH SOLUTION, TRAVERSE THE GRAPH FROM THESTART NODE:",self.start)
    ...
    def aoStar(self, v, backTracking): # AO* algorithm for a start node and backTracking status flag
        ...
        if self.getStatus(v) >= 0: # if status node v >=0, compute Minimum Cost nodes of v
            ...
            if solved==True: # if the Minimum Cost nodes of v are solved, set the current node status as solved(-1)
                self.setStatus(v,-1)
                self.solutionGraph[v]=childNodeList # update thesolution graph with the solved nodes which may be a part of solution
